...
vector <string> fileReadAllLines(string file){vector <string> lines;ifstream myfile;myfile.open(file.c_str());if (myfile.is_open()){ while ( myfile.good() ){string line; getline (myfile,line);lines.push_back(line);}myfile.close();}return lines;}
...
vstring dirFilesIncludingSubdirs(string srcdir,string exts ){
  autoExpandEnvironmentVariables(srcdir);
	...
	filesystem::path current_dir(srcdir);
	vstring res;
	res.reserve(500);
	string fn;
	trim(exts,";");
