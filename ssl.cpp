...
// string site="nunofcguerreiro.com";
// string request="/api-euromillions-json?result=all";
// sslconnect ssl(site,443);
// std::string fullResuest = "GET " + request + " HTTP/1.1\r\n";
// fullResuest+= "Host: " + site + "\r\n";
// fullResuest += "Accept: */*\r\n";
// fullResuest += "Connection: close\r\n\r\n";
// cot(ssl.get(fullResuest,0));
// https://stackoverflow.com/questions/38714363/read-html-response-using-ssl-read-in-c-http-1-0
string sslconnect::get(string hh,bool dbg){
	if(dbg)printf("%s\n",hh.c_str());
	int er; 
	er = SSL_write (ssl, hh.c_str(), hh.size());
	CHK_SSL(er);
	...
	er = SSL_read(ssl, buf, sizeof(buf));
	while (er > 0) {
		write(STDOUT_FILENO, buf, er);
		memset(buf, '\0', sizeof(buf));
		er = SSL_read(ssl, buf, sizeof(buf));
	}
	return buf;
...
//	dbgv(hostname,porti);
	struct hostent *host;
	if((host=gethostbyname(hostname.c_str()))==0){printf("errorhost\n");return;}
